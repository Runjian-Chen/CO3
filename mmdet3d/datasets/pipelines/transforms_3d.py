...
@PIPELINES.register_module()
class RandomFlip3D_LIDAR_ONLY(RandomFlip):
    """Flip the points & bbox.
    ...
    def __init__(self,
                 ...
                 flip_ratio_bev_vertical=0.0,
                 **kwargs):
        super(RandomFlip3D_LIDAR_ONLY, self).__init__(
            flip_ratio=flip_ratio_bev_horizontal, **kwargs)
        self.sync_2d = sync_2d
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
        if flip_ratio_bev_horizontal is not None:
...
class GlobalRotScaleTrans(object):
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        if 'bbox3d_fields' in input_dict.keys():
            for key in input_dict['bbox3d_fields']:
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        input_dict['noise_rotation'] = torch.tensor(noise_rotation)
        # if no bbox in input_dict, only rotate points
        if (not 'bbox3d_fields' in input_dict.keys()) or len(input_dict['bbox3d_fields']) == 0:
...
@PIPELINES.register_module()
class GlobalRotScaleTrans_STRL(object):
    def __init__(self,
                 rot_range=[-0.78539816, 0.78539816],
                 scale_ratio_range=[0.95, 1.05],
                 translation_std=[0, 0, 0],
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        input_dict['t0_points'].translate(trans_factor)
        input_dict['t_plus_points'].translate(trans_factor)
        input_dict['pcd_trans'] = trans_factor
        if 'bbox3d_fields' in input_dict.keys():
            for key in input_dict['bbox3d_fields']:
                input_dict[key].translate(trans_factor)
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        # if no bbox in input_dict, only rotate points
        if (not 'bbox3d_fields' in input_dict.keys()) or len(input_dict['bbox3d_fields']) == 0:
            rot_mat_T = input_dict['t0_points'].rotate(noise_rotation)
            rot_mat_T = input_dict['t_plus_points'].rotate(noise_rotation)
        ...
        for key in input_dict['bbox3d_fields']:
            if len(input_dict[key].tensor) != 0:
                t0_points, rot_mat_T = input_dict[key].rotate(
                    noise_rotation, input_dict['t0_points'])
                t_plus_points, rot_mat_T = input_dict[key].rotate(
                    noise_rotation, input_dict['t_plus_points'])
                input_dict['t0_points'] = t0_points
                input_dict['t_plus_points'] = t_plus_points
                input_dict['pcd_rotation'] = rot_mat_T
    ...
    def _scale_bbox_points(self, input_dict):
        ...
        t0_points = input_dict['t0_points']
        t0_points.scale(scale)
        input_dict['t0_points'] = t0_points

        t_plus_points = input_dict['t_plus_points']
        t_plus_points.scale(scale)
        input_dict['t_plus_points'] = t_plus_points
...
class AugmentPoints(object):
    ...
    def __init__(self,
        ...
        self.random_flip = RandomFlip3D(sync_2d, flip_ratio_bev_horizontal, flip_ratio_bev_vertical)
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        input_dict['aug_points'].translate(trans_factor)
        input_dict['aug_points_trans'] = trans_factor
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        rotation = self.rot_range
        noise_rotation = np.random.uniform(rotation[0], rotation[1])
        aug_rot_mat_T = input_dict['aug_points'].rotate(noise_rotation)
        input_dict['aug_rot_mat_T'] = aug_rot_mat_T
    ...
    def _scale_bbox_points(self, input_dict):
        ...
        Returns:
            dict: Results after scaling, 'points'and keys in \
                input_dict['bbox3d_fields'] are updated in the result dict.
        """
        scale = input_dict['aug_points_scale_factor']
        aug_points = input_dict['aug_points']
        aug_points.scale(scale)

        input_dict['aug_points'] = aug_points
    ...
    def _random_scale(self, input_dict):
        ...
        scale_factor = np.random.uniform(self.scale_ratio_range[0],
                                         self.scale_ratio_range[1])
        input_dict['aug_points_scale_factor'] = scale_factor

    def __call__(self, input_dict):
        """Private function to get an augmented version of the point cloud with rotate, scale, translate and random flip
        ...
        Returns:
            dict: Results after scaling, 'points', 'pcd_rotation',
                ...
                input_dict['bbox3d_fields'] are updated in the result dict.
        """
        input_dict['aug_points'] = input_dict['points'].clone()
        input_dict['voxel_size'] = self.voxel_size
        input_dict['point_cloud_range'] = self.point_cloud_range

        if 'aug_points_transformation_3d_flow' not in input_dict:
            input_dict['aug_points_transformation_3d_flow'] = []

        self._rot_bbox_points(input_dict)

        if 'aug_points_scale_factor' not in input_dict:
            self._random_scale(input_dict)
        self._scale_bbox_points(input_dict)

        self._trans_bbox_points(input_dict)

        ## ToDo: Get random flip involved

        input_dict['aug_points_transformation_3d_flow'].extend(['R', 'S', 'T'])
...
@PIPELINES.register_module()
class PointsRangeFilter_DAIR(object):
    """Filter points by the range.
    ...
    def __init__(self, pcd_limit_range_vehicle, pcd_limit_range_fusion):
        self.pcd_limit_range_vehicle = np.array(pcd_limit_range_vehicle, dtype=np.float32)
        self.pcd_limit_range_fusion = np.array(pcd_limit_range_fusion, dtype=np.float32)
    ...
    def __call__(self, input_dict):
        ...
        if 'vehicle_to_infrastructure_idx' in input_dict.keys():
            vehicle_to_infrastructure_idx = input_dict['vehicle_to_infrastructure_idx']
        if 'vehicle_shape_context_feature' in input_dict.keys():
            vehicle_shape_context_feature = input_dict['vehicle_shape_context_feature']

        vehicle_points_mask = self.in_range_3d(self.pcd_limit_range_vehicle, vehicle_points)
        input_dict['vehicle_points'] = vehicle_points[vehicle_points_mask]
        if 'vehicle_to_infrastructure_idx' in input_dict.keys():
            input_dict['vehicle_to_infrastructure_idx'] = vehicle_to_infrastructure_idx[vehicle_points_mask]
        if 'vehicle_shape_context_feature' in input_dict.keys():
            input_dict['vehicle_shape_context_feature'] = vehicle_shape_context_feature[vehicle_points_mask]

        fusion_points = torch.cat([vehicle_points, infrastructure_points], dim=0)
        fusion_points_infrastructure_mask = torch.cat([torch.zeros(vehicle_points.shape[0]), torch.ones(infrastructure_points.shape[0])])
        fusion_points_mask = self.in_range_3d(self.pcd_limit_range_fusion, fusion_points)
        fusion_points = fusion_points[fusion_points_mask, :]
        fusion_points_infrastructure_mask = fusion_points_infrastructure_mask[fusion_points_mask]
        shuffled_idx = torch.randperm(fusion_points.shape[0])
        fusion_points = fusion_points[shuffled_idx, :]
        fusion_points_infrastructure_mask = fusion_points_infrastructure_mask[shuffled_idx]
        input_dict['fusion_points_shuffled_idx'] = shuffled_idx
        input_dict['fusion_points'] = fusion_points
        input_dict['fusion_points_infrastructure_mask'] = fusion_points_infrastructure_mask
        input_dict['n_infrastructure_points'] = torch.sum(fusion_points_infrastructure_mask)
        input_dict['infrastructure_points'] = infrastructure_points[self.in_range_3d(self.pcd_limit_range_fusion, infrastructure_points)]
...
@PIPELINES.register_module()
class PointsRangeFilter_DAIR_STRL(object):
    """Filter points by the range.
    ...
    def __call__(self, input_dict):
        ...
        t0_points = t0_points.tensor
        t_plus_points = t_plus_points.tensor

        t0_points_mask = self.in_range_3d(self.pcd_limit_range_vehicle, t0_points)
        input_dict['t0_points'] = t0_points[t0_points_mask]

        t_plus_points_mask = self.in_range_3d(self.pcd_limit_range_vehicle, t_plus_points)
        input_dict['t_plus_points'] = t_plus_points[t_plus_points_mask]
