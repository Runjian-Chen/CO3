...
class KittiDatasetCrossModalPretraining(Custom3DDataset):
    r"""KITTI Dataset.
    ...
    def prepare_train_data(self, index):
        ...
        label = gt_bboxes_3d.data.points_in_boxes_cpu(example['points'].data[:,:3])
        if label.shape[0] <= 1 or len(label.shape) == 1:
            return None
        else:
            cnt_obj = 0
            for obj in range(label.shape[0]):
                if label[obj,:].bool().any():
                    cnt_obj += 1
            if cnt_obj >= 3:
                return example
            else:
