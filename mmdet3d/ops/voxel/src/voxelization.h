...
struct voxel_num_with_voxel_idx
{
   int voxel_num;
   at::Tensor voxel_idx;
};
...
int hard_voxelize_gpu_with_voxel_idx(const at::Tensor &points, at::Tensor &voxels,
                      at::Tensor &coors, at::Tensor &num_points_per_voxel,
                      const std::vector<float> voxel_size,
                      const std::vector<float> coors_range,
...
// ToDo: return voxel index in all the possible voxelization methods
inline int hard_voxelize_with_voxel_idx(const at::Tensor &points, at::Tensor &voxels,
                         at::Tensor &coors, at::Tensor &num_points_per_voxel,
                         const std::vector<float> voxel_size,
                         const std::vector<float> coors_range,
                         const int max_points, const int max_voxels,
                         at::Tensor &voxels_coors,
                         const int NDim = 3, const bool deterministic = true) {

//      std::cout << std::type_info(voxels_coors[0][0]) << std::endl;
//      voxelization_return.voxel_idx = voxelization_return.voxel_idx.contiguous();
//      voxels_coors = voxels_coors.contiguous();
//      memcpy(voxels_coors.data_ptr(), voxelization_return.voxel_idx.data_ptr(), sizeof(torch::kInt));
//      memcpy(voxels_coors.data_ptr(), voxelization_return.voxel_idx.data_ptr(), sizeof(torch::kInt));

//      for (int i = 0; i < voxelization_return.voxel_idx.size(0) ; i++)
//      {
//        for (int j = 0; j < 3 ; j++)
//        {
//           voxels_coors[i][j] = voxelization_return.voxel_idx[i][j];
//        }
//      }

      return hard_voxelize_gpu_with_voxel_idx(points, voxels, coors, num_points_per_voxel,
                               voxel_size, coors_range, voxels_coors, max_points, max_voxels,
                               NDim);
}
