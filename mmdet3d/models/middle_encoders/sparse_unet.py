...
class SparseUNet(BaseModule):
    ...
    def __init__(self,
        ...
        if pretrained is not None and isinstance(pretrained, str):
            ...
            model_dict = self.conv_input.state_dict()
            ...
            self.conv_input.load_state_dict(model_dict)

            model_dict = self.encoder_layers.state_dict()
            pretrained_dict = {k[len('pts_middle_encoder.encoder_layers.'):]: v for k, v in checkpoint.items() if
                               k.startswith('pts_middle_encoder.encoder_layers.')}

            model_dict.update(pretrained_dict)
            self.encoder_layers.load_state_dict(model_dict)

        elif pretrained is not None:
            raise TypeError('pretrained must be a str or None')

    @auto_fp16(apply_to=('voxel_features', ))
    def forward(self, voxel_features, coors, batch_size, return_spconv_feats = False):
        ...
        if return_spconv_feats:
            return encode_features[-1].dense()
        # for detection head
        # [200, 176, 5] -> [200, 176, 2]
...
@MIDDLE_ENCODERS.register_module()
class SparseUNet_C2D2(BaseModule):
    r"""SparseUNet for PartA^2.
    ...
    def forward(self, voxel_features, coors, batch_size, return_spconv_feats = False):
        ...
        seg_features = decode_features[-1].features
        seg_indices = decode_features[-1].indices

        ret = dict(
            spatial_features=spatial_features, seg_features=seg_features, seg_indices=seg_indices)
