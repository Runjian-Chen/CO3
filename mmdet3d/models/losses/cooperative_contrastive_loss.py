...
class cooperative_contrastive_loss(nn.Module):

    def __init__(self, temperature = 0.7, sample_num=2048, fileter_ground_points = False):
        super(cooperative_contrastive_loss, self).__init__()

        self.temperature = temperature
        self.sample_num = sample_num
        self.fileter_ground_points = fileter_ground_points
        self.log_var = {}
        self.loss_func_name = 'cooperative_contrastive_loss'
    ...
    def forward(self,
        ...
        cooperative_contrastive_loss = torch.zeros(1).to(dense_vehicle_voxel_feats.device)
        ...
        for b in range(batch_size):
            ...
            cooperative_contrastive_loss += torch.mean(-1.0 * torch.log(s_pos / s_sum))
        ...
        return cooperative_contrastive_loss