...
def point_sample(img_meta,
    ...
    if not return_pixel_pos:
        return point_features.squeeze().t()
    else:
        return point_features.squeeze().t(), grid.squeeze()
...
@FUSION_LAYERS.register_module()
class Point_Pixel_Correspondence(BaseModule):
    """Fuse image features from multi-scale features.
    ...
    def __init__(self,
                 ...
                 padding_mode='zeros',
                 lateral_conv=True,
                 return_pixel_pos = True):
        super(Point_Pixel_Correspondence, self).__init__(init_cfg=init_cfg)
        if isinstance(img_levels, int):
            img_levels = [img_levels]
        if isinstance(img_channels, int):
            img_channels = [img_channels] * len(img_levels)
        ...
        self.return_pixel_pos = return_pixel_pos
    ...
    def forward(self, img_feats, pts, img_metas):
        ...
        Returns:
            torch.Tensor: Fused features of each point.
        """
        if self.return_pixel_pos:
            img_feats_per_point, pixel_pos_per_point = self.obtain_mlvl_feats(img_feats, pts, img_metas, self.return_pixel_pos)
            return img_feats_per_point, pixel_pos_per_point
        else:
            img_feats_per_point = self.obtain_mlvl_feats(img_feats, pts, img_metas, self.return_pixel_pos)
            return img_feats_per_point

    def obtain_mlvl_feats(self, img_feats, pts, img_metas, return_pixel_pos = False):
        """Obtain multi-level features for each point.
        ...
        for i in range(len(img_metas)):
            ...
            for level in range(len(self.img_levels)):
                if not return_pixel_pos:
                    mlvl_img_feat = self.sample_single(img_ins[level][i:i + 1], pts[i][:, :3],
                                       img_metas[i], return_pixel_pos)
                else:
                    mlvl_img_feat, pixel_pos_pts = self.sample_single(img_ins[level][i:i + 1], pts[i][:, :3],
                                       img_metas[i], return_pixel_pos)
                mlvl_img_feats.append(
                    mlvl_img_feat)

            mlvl_img_feats = torch.cat(mlvl_img_feats, dim=-1)
            img_feats_per_point.append(self.img_transform(mlvl_img_feats))
            if return_pixel_pos:
                pixel_pos_per_point.append(pixel_pos_pts)
    ...
    def sample_single(self, img_feats, pts, img_meta, return_pixel_pos = False):
        """Sample features from single level image feature map.
        ...
        if not return_pixel_pos:
            img_pts = point_sample(
                ...
                padding_mode=self.padding_mode,
                align_corners=self.align_corners,
                return_pixel_pos = return_pixel_pos
            )
            return img_pts
        else:
            img_pts, pixel_pos_pts = point_sample(
                img_meta=img_meta,
                img_features=img_feats,
                points=pts,
                proj_mat=pts.new_tensor(proj_mat),
