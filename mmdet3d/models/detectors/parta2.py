...
class PartA2(TwoStage3DDetector):
    ...
    def voxelize_with_voxels_coors(self, points):
        ...
        """
        voxels, coors, num_points, voxels_coors, voxel_centers  = [], [], [], [], []
        for res in points:
            # ToDo: make return_voxel_idx faster
            res_voxels, res_coors, res_num_points, res_voxels_coors = self.voxel_layer(res, return_voxel_idx = True)
            res_voxel_centers = (
                                        res_coors[:, [2, 1, 0]] + 0.5) * res_voxels.new_tensor(
                self.voxel_layer.voxel_size) + res_voxels.new_tensor(
                self.voxel_layer.point_cloud_range[0:3])
        ...
        return voxel_dict, voxels_coors

    @torch.no_grad()
    def extract_3d_representation(self, points, img = None, img_metas = None):

        voxel_dict, voxels_coors = self.voxelize_with_voxels_coors(points)

        voxel_features = self.voxel_encoder(voxel_dict['voxels'],
                                            voxel_dict['num_points'],
                                            voxel_dict['coors'])
        batch_size = voxel_dict['coors'][-1, 0].item() + 1
        voxels_feat = self.middle_encoder(voxel_features, voxel_dict['coors'],
                                         batch_size, return_spconv_feats = True)

        voxels_feat = voxels_feat[0]
        voxels_coors = voxels_coors[0] // 8
        sampled_coors = voxels_coors.long()
        sampled_D, sampled_H, sampled_W = sampled_coors[:, 0], sampled_coors[:, 1], sampled_coors[:, 2]
        sampled_voxel_feats = voxels_feat[:, sampled_D, sampled_H, sampled_W]
        sampled_voxel_feats = sampled_voxel_feats.permute(1, 0)

        return sampled_voxel_feats
